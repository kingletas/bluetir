...
module Bluetir
  ...
  class Acceptance
    ...
    def snapshot(store)
      probe = Probe.new(@context, @orders)
      sightings = probe.run
      Baseline.capture(store: store, landings: probe.landings, sightings: sightings,
                       titles: probe.titles)
    end
    ...
    def record(drift)
      regression = Baseline.regression?(drift)
