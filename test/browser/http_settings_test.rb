...
class HttpSettingsTest < Minitest::Test
  def test_it_identifies_the_tool_by_default
    settings = Bluetir::Browser::HttpSettings.from(nil)
  ...
  def test_it_is_polite_by_default
    assert_in_delta(1.0, Bluetir::Browser::HttpSettings.from(nil).delay)
  end

  def test_it_takes_a_user_agent_and_a_delay
    settings = Bluetir::Browser::HttpSettings.from('user_agent' => 'Mozilla/5.0', 'delay' => 0.25)
  ...
  def test_headers_become_strings
    settings = Bluetir::Browser::HttpSettings.from('headers' => { 'X-Debug' => 1 })
  ...
  def test_params_become_strings
    settings = Bluetir::Browser::HttpSettings.from('params' => { '___store' => :default })
  ...
  def test_nothing_configured_means_nothing_to_apply
    settings = Bluetir::Browser::HttpSettings.from({})
  ...
  def test_a_cookie_without_a_name_is_refused
    assert_raises(Bluetir::ConfigurationError) do
      Bluetir::Browser::HttpSettings.from('cookies' => [{ 'value' => 'x' }])
    end
  end

  def test_a_cookie_that_is_not_a_mapping_is_refused
    assert_raises(Bluetir::ConfigurationError) do
      Bluetir::Browser::HttpSettings.from('cookies' => ['store=default'])
    end
  end

  def test_local_storage_becomes_strings
    settings = Bluetir::Browser::HttpSettings.from('local_storage' => { 'seen_notice' => true })
  ...
  def test_nothing_stored_means_nothing_to_seed
    refute_predicate Bluetir::Browser::HttpSettings.from({}), :local_storage?
  end

  def test_the_summary_says_what_is_configured
    settings = Bluetir::Browser::HttpSettings.from('headers' => { 'A' => 'b' }, 'delay' => 2)
