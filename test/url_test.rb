...
class UrlTest < Minitest::Test
  def test_it_joins_a_relative_path
    assert_equal 'https://s.test/cart', Bluetir::Url.new('https://s.test').for('cart')
  end
  ...
  def test_an_empty_path_is_the_store_itself
    assert_equal 'https://s.test', Bluetir::Url.new('https://s.test').for('')
    assert_equal 'https://s.test', Bluetir::Url.new('https://s.test').for(nil)
  end

  def test_an_absolute_url_is_left_alone
    url = Bluetir::Url.new('https://s.test')

    assert_equal 'https://other.test/x', url.for('https://other.test/x')
  end

  def test_it_appends_the_configured_params
    url = Bluetir::Url.new('https://s.test', params: { '___store' => 'de' })
  ...
  def test_it_encodes_a_param_that_needs_it
    url = Bluetir::Url.new('https://s.test', params: { 'q' => 'a b&c' })
