...
class NotifierTest < Minitest::Test
  def test_it_is_not_configured_without_recipients
    notifier = Bluetir::Notifier.new(recipients: [], sender: 'a@b.test', server: 'localhost')

    refute_predicate notifier, :configured?
  end

  def test_it_is_not_configured_without_a_server
    notifier = Bluetir::Notifier.new(recipients: ['a@b.test'], sender: 'a@b.test', server: nil)

    refute_predicate notifier, :configured?
  end

  def test_it_sends_nothing_when_nobody_is_configured
    notifier = Bluetir::Notifier.new(recipients: [], sender: 'a@b.test', server: nil)

    refute notifier.deliver(Bluetir::Result.new, 'https://store.test')
  end
  ...
  def test_the_subject_refuses_to_call_an_empty_run_a_success
    assert_match(/nothing was proven/, subject_for(Bluetir::Result.new))
  end

  private

  def subject_for(result)
    notifier = Bluetir::Notifier.new(recipients: ['a@b.test'], sender: 'a@b.test', server: 'x')
